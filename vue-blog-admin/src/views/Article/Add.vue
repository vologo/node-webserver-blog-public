      ...
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" width="100%"
        ...
        <el-form-item label="封面" prop="cover">
          <el-upload :action="uploadUrl" list-type="picture-card" name="file"
            :limit="1" :data="{bucket}" :on-preview="handlePictureCardPreview"
        ...
        <el-form-item label="内容" prop="content">
          <mavon-editor ref="editor" :ishljs="true" v-model="ruleForm.content"
            @imgAdd="imgAdd" />
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    successPic(response, file, fileList) {
      ...
      // console.info(1111, response, file, fileList);
    },
    ...
    submit() {
      ...
      let url = `${this.API}/api/article/create`;
      this.Http.$post(url, "post", parmas, {}).then(
        res => {
          if (res.code != 200) {
